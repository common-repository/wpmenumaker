...
class Helpers {
    ...
    public static function GetHoursPopUp() {
    ...
    public static function getSessionOrder() {
        return Yii::app()->session['wpmm_order'];
    }
    ...
    public static function getWorkingHours($time_string) {
    ...
    public static function validateWorkingHours($time_string) {
        $timestamp = strtotime($time_string);
        if (!$timestamp)
            return false;
        if ($timestamp < time())
    ...
    public static function sendOrderToEmail($order,$email,$full=false){
       ...
       $message->setBody(Yii::app()->controller->renderPartial('//admin/_orderblank', array('model' => $order, 'full' => $full), true).self::shopDetails(), 'text/html', 'utf-8');
       $message->addTo($email);
    ...
    public static function sendRegisterMessage($user, $pass) {
        YiiBase::import('ext.YiiMail.YiiMailMessage');
        $shopemail = MMSettingsForm::getParam('vendor_email');
        if(empty($shopemail)) $shopemail='noemail@noemail.mail';
        $shopname = MMSettingsForm::getParam('vendor_name');
        if(empty($shopname)) $shopname='Unknown shop';
        $message = new YiiMailMessage;
        $message->setBody('<b>Dear Customer</b><br>You have registered on to :<b>' . $shopname . '</b> by submiting order from us. Your account is<br>Username: ' . $user->c_mail . '<br>Password: ' . $pass . '<br><b>Thank you for using our service!</b> ' . self::shopDetails(), 'text/html', 'utf-8');
    ...
    public static function shopDetails(){
        $message='<br><hr><br>If you have questions, ask us by any of this contacts:';
        $email=MMSettingsForm::getParam('vendor_email');
        $phone=MMSettingsForm::getParam('vendor_phone');
        $fax=MMSettingsForm::getParam('vendor_fax');
        if(!empty($email))
    ...
    public static function renderSelectedAttribute($attribute) {
