 ...
 */
Yii::import('application.components.boot.TbaGridView');
 ...
 *  - Sortable rows
 */
class TbaExtendedGridView extends TbaGridView
{
	/**
	...
	public function renderChart()
	{
		...
		$this->componentsReadyScripts[] = 'jQuery(document).on("click",".' . $this->getId() . '-grid-control", function(){
			if(jQuery(this).hasClass("grid") && jQuery("#' . $this->getId() . ' #' . $chartId . '").is(":visible"))
			{
				jQuery("#' . $this->getId() . ' #' . $chartId . '").hide();
				jQuery("#' . $this->getId() . ' table.items").show();
			}
			if(jQuery(this).hasClass("chart") && jQuery("#' . $this->getId() . ' table.items").is(":visible"))
			{
				jQuery("#' . $this->getId() . ' table.items").hide();
				jQuery("#' . $this->getId() . ' #' . $chartId . '").show();
			}
			return false;
		});';
		// end switch buttons
		...
		if ($this->ajaxUpdate !== false)
		{
			...
			$this->componentsAfterAjaxUpdate[] = "highchart{$chartId} = new Highcharts.Chart(jQuery('#{$chartId}').data('config'));";
		}
		$configChart = array(
	...
	public function registerCustomClientScript()
	{
		...
		if ($this->fixedHeader)
		{
			...
			$fixedHeaderJs = "jQuery('#{$this->id} table.items').stickyTableHeaders({fixedOffset:{$this->headerOffset}});";
		...
		if ($this->sortableRows)
		{
			...
			$this->componentsReadyScripts[] = "jQuery.fn.yiiGridView.sortable('{$this->id}', '{$sortableAction}', {$afterSortableUpdate});";
			$this->componentsAfterAjaxUpdate[] = "jQuery.fn.yiiGridView.sortable('{$this->id}', '{$sortableAction}', {$afterSortableUpdate});";
		}
		...
		if($this->selectableCells)
		{
			...
			$this->componentsReadyScripts[] = "jQuery.fn.yiiGridView.selectable('{$this->id}','{$this->selectableCellsFilter}',{$afterSelectableCells});";
			$this->componentsAfterAjaxUpdate[] = "jQuery.fn.yiiGridView.selectable('{$this->id}','{$this->selectableCellsFilter}', {$afterSelectableCells});";
		}

		$cs->registerScript(__CLASS__ . '#' . $this->id . 'Ex', '
			$grid = jQuery("#' . $this->id . '");
			' . $fixedHeaderJs . '
			if(jQuery(".' . $this->extendedSummaryCssClass . '", $grid).length)
			{
				jQuery(".' . $this->extendedSummaryCssClass . '", $grid).html(jQuery("#' . $this->id . '-extended-summary", $grid).html());
			}
			' . (count($this->componentsReadyScripts) ? implode(PHP_EOL, $this->componentsReadyScripts) : '') . '
			jQuery.ajaxPrefilter(function (options, originalOptions, jqXHR) {
				var qs = jQuery.deparam.querystring(options.url);
				if(qs.hasOwnProperty("ajax") && qs.ajax == "' . $this->id . '")
				{
					...
					options.success = function(data)
					{
						if(options.realsuccess) {
							...
							if(jQuery(".' . $this->extendedSummaryCssClass . '", jQuery("#' . $this->id . '")))
							{
								jQuery(".' . $this->extendedSummaryCssClass . '", jQuery("#' . $this->id . '")).html(jQuery("#' . $this->id . '-extended-summary", $data).html());
							}
...
class TbPercentOfTypeGooglePieOperation extends TbPercentOfTypeOperation
{
	...
	public function registerClientScript()
	{
		...
		$this->column->grid->componentsAfterAjaxUpdate[__CLASS__] =
			'var $el = jQuery("#' . $this->getId() . '");var data = $el.data("data");var opts = $el.data("options");
...
class TbPercentOfTypeEasyPieOperation extends TbPercentOfTypeOperation
{
	...
	public $chartOptions = array(
		...
		'onStart' => 'js:jQuery.noop', // Callback function that is called at the start of any animation (only if animate is not false).
		'onStop' => 'js:jQuery.noop' // Callback function that is called at the end of any animation (only if animate is not false).
	);
	...
	protected function registerClientScripts()
	{
		...
		Yii::app()->getClientScript()->registerScript(__CLASS__ . '#percent-of-type-operation-simple-pie', '
			jQuery("#' . $this->column->grid->id . ' .' . $this->column->grid->extendedSummaryCssClass . ' .' . $this->chartCssClass . '")
