<?php
    ...
    $hours_popup = Helpers::GetHoursPopUp();
...
<div id="wpmm-form-container-scroll">
        ...
        ?>
            ...
            ?>
                ...
                <div class="wpmm-form-item-extra" data-key="<?php echo $key ?>">
                    <?php if (count($order_item['attributes'])) : ?>
                            ...
                            <?php foreach ($order_item['attributes'] as $attr_id => $attr_price) :
                                ...
                                <tr>
                                    ...
                                    <td class="wpmm-form-attr-price">
                                        <?php
                                            if (substr($attr_price, 0, 1) === '-')
                                                echo '- $' . number_format((float) substr($attr_price, 1), 2);
                                            else
                                                echo '+ $' . number_format((float) $attr_price, 2);
                                        ?>
                                    </td>
                                </tr>
                            <?php endforeach ?>
    ...
    <div id="wpmm-form-delivery">
        ...
        ?>
        <a class="wpmm-green-btn" id="wpmm-form-delivery-info" href="#" title="<?php echo $hours_popup; ?>"><?php if($enable_delivery) echo 'Restaurant Delivery Info'; else echo 'Restaurant Info'; ?></a>
        <hr class="wpmm-form-hr">
        <?php
        ...
        <div align="center">
        <?php echo CHtml::label('Select Addres', 'address') ?><br>
            <?php echo CHtml::dropDownList('address', '', Helpers::GetAddressList(),array('empty'=>'Add new address...','id'=>'wpmm-address-dropdown','options'=>array($cartform['address']=>array('selected'=>true)))); ?>
            <?php if(!empty($cartform['address'])){
...
<div id="wpmm-cart-control" align="center">
    ...
    <a href="#" id="wpmm-form-delete" title="Delete order"></a>
</div>
    <?php $this->endWidget() ?>
<script>
    function fixHeight() {
        ...
        jQuery.ajax({
            ...
            success: function(data) {
                if (data) {
                    ...
                    details.show();
                    details.html(data);
                    details.css({'top':'50%','left':'50%','margin':'-'+(details.height())+'px'+' 0 0 -'+(details.width() / 2)+'px'});
                }
            }
        });
        return false;
    ...
    function save_tip() {
        switch (jQuery('#wpmm-form-tip input[type=radio]:checked').attr('id')) {
            case 'wpmm-tip-percent': var type = 'percent'; break;
            case 'wpmm-tip-amount': var type = 'amount'; break;
        }
        var value = jQuery('#wpmm-tip-value').val();
